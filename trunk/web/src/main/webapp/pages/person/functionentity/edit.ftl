...
<div class="form-title"><span class="form-title-into">编辑功能实体</span></div>
     ...
     <@s.component id="select_functionEntityType" template="components/dropdown.ftl"> 
		...
		<@s.param name="defaultOptionLabel" value="${'entity.functionEntityType.name'}" />
		<@s.param name="defaultOptionValue" value="${'entity.functionEntityType.id'}" />
     </@s.component>


<@s.form id="saveform" name="saveform" action="">
   
   <@s.hidden id="entityId" name="entity.id" value="%{entity.id}"/>
   <@s.hidden name="entity.version" value="%{entity.version}"/>
   <@s.hidden name="entity.status" value="%{entity.status}"/>
   <@s.hidden id="functionEntityType" value="%{entity.functionEntityType.id}"/>

  <table cellspacing="1" cellpadding="0" class="form-2column form-margin">
    <tr>
      <th>功能实体名称：</th>
      <td class="from-require"><input type="text" id="entity.name" name="entity.name" value="${entity.name}" dataType="Require" msg="请填写功能实体名称" autocomplete="off" class="input-common input-word-10" />
例如“com.ce.myproject.MyService，作为功能实体真正的身份标志”</td>
    </tr>
    <tr>
      <th>功能实体别名：</th>
      <td class="from-require"><input type="text" name="entity.alias" value="${entity.alias}" dataType="Require" msg="请填写功能实体别名" autocomplete="off" class="input-common input-word-10" />
例如“我的方法”，作为树显示使用”</td>
    </tr>
    <tr>
      <th>功能实体类型：</th>
      <td><select id="select_functionEntityType" name="entity.functionEntityType.id" class="select-word-10">
	  </select></td>
    </tr>
    <tr>
      <th>父实体名称：</th>
	  ...
	  <input id="fpid" type="hidden" name="entity.parent.id" value="${entity.parent.id }" />
    ...
    <tr>
      <th>备注：</th>
      <td><textarea name="entity.description" cols="50" rows="5">${entity.description}</textarea></td>
    </tr>
  </table>
